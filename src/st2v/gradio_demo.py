...
def generate(
    ...
    if model_name_stage1 == "ModelScopeT2V (text to video)":
        ...
        short_video = svd_short_gen(
            image, prompt, svd_model, sdxl_model, inference_generator, t, device
        )
...
def enhance(
    ...
    num_frames=None,
    image=None,
    model_name_stage1=None,
    model_name_stage2=None,
    seed=33,
    t=50,
    image_guidance=9.5,
    result_fol=result_fol,
):
    if input_to_enhance is None:
        input_to_enhance = generate(
            prompt,
            num_frames,
            image,
            model_name_stage1,
    ...
    if on_huggingspace:
        encoded_video = video2video(
            prompt, input_to_enhance, result_fol, cfg_v2v, msxl_model
        )
    else:
        # Randomized blending is meaningful mainly for long videos
        encoded_video = video2video_randomized(
            prompt, input_to_enhance, result_fol, cfg_v2v, msxl_model
        )
...
def change_visibility(value):
    if value == "SVD (image to video)":
        return gr.Image(
            label="Image Prompt (if not attached then SDXL will be used to generate the starting image)",
            show_label=True,
            scale=1,
        ...
        return gr.Image(
            label="Image Prompt (first select Image-to-Video model from advanced options to enable image upload)",
            show_label=True,
            scale=1,
            show_download_button=False,
            interactive=False,
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                    ...
                    with gr.Row():
                        num_frames = gr.Dropdown(
                            [
                                ...
                                "48 - frames",
                                "56 - frames",
                                "80 - recommended to run on local GPUs",
                                "240 - recommended to run on local GPUs",
                                "600 - recommended to run on local GPUs",
                                "1200 - recommended to run on local GPUs",
                                "10000 - recommended to run on local GPUs",
                            ],
                            label="Number of Video Frames",
                            info="For >56 frames use local workstation!",
                            value="24 - frames",
                        )
                    with gr.Row():
                        prompt_stage1 = gr.Textbox(
                            label="Textual Prompt",
                            placeholder="Ex: Dog running on the street.",
                        )
                    with gr.Row():
                        image_stage1 = gr.Image(
                            label="Image Prompt (first select Image-to-Video model from advanced options to enable image upload)",
                            show_label=True,
                            scale=1,
                            show_download_button=False,
                ...
                with gr.Column():
                    video_stage1 = gr.Video(
                        label="Long Video Preview",
                        show_label=True,
                        interactive=False,
                        scale=2,
            ...
            with gr.Row():
                with gr.Row():
                    run_button_stage1 = gr.Button(
                        "long Video Generation (faster preview)"
                    )
                with gr.Row():
    ...
    model_name_stage1.change(
        fn=change_visibility, inputs=[model_name_stage1], outputs=image_stage1
    )
    ...
    run_button_stage1.click(
        fn=generate,
        inputs=inputs_t2v,
        outputs=video_stage1,
    )
