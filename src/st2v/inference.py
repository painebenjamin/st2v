...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--chunk", type=int, default=56, help="chunk_size for randomized blending"
    )
    parser.add_argument(
        "--overlap", type=int, default=32, help="overlap_size for randomized blending"
    )
    ...
    stream_cli, stream_model = init_streamingt2v_model(
        ckpt_file_streaming_t2v, result_fol
    )
    ...
    if args.num_frames > 80:
        ...
        video2video(
            args.prompt, opj(result_fol, name + ".mp4"), result_fol, cfg_v2v, msxl_model
        )
