...
class FrameConditioning:
    ...
    def attach_video_frames(
        ...
        with torch.no_grad():
            ...
            assert (
                n_frames_inference == z_0.shape[1]
            ), "For frame injection, the number of frames sampled by the dataloader must match the number of frames used for video generation"
            shape = list(z_0.shape)

            shape[1] = pl_module.inference_params.video_length
            M = torch.zeros(
                shape, dtype=z_0.dtype, device=pl_module.device
            )  # [B F C W H]
            bsz = z_0.shape[0]
            if random_mask:
                p_inject_frame = self.injection_probability
                use_masks = torch.bernoulli(
                    torch.tensor(p_inject_frame).repeat(bsz)
                ).long()
                keep_frame_idx = torch.randint(
                    0, n_frames_inference, (bsz,), device=pl_module.device
                ).long()
            else:
            ...
            for batch_idx, (keep_frame, use_mask) in enumerate(
                zip(keep_frame_idx, use_masks)
            ):
            ...
            if (
                getattr(pl_module.opt_params.noise_decomposition, "use", False)
                and random_mask
            ):
                assert (
                    x0.shape[0] == 1
                ), "randomizing frame injection with noise decomposition not implemented for batch size >1"
        return x0, frame_idx
...
class NoiseDecomposition:
    ...
    def get_loss(
        ...
        if self.use_base_model:
            base_pred = unet_base(
                z_t_base, timesteps, encoder_hidden_states, x0=x0_base
            ).sample
        else:
        ...
        base_correction = (
            math.sqrt(lambda_f) * torch.sqrt(1 - timesteps_alphas) * base_pred
        )

        z_t_residual_dash = z_t_residual - base_correction

        residual_pred = unet(
            z_t_residual_dash, timesteps, encoder_hidden_states, x0=x0_residual
        ).sample
        composed_pred = (
            math.sqrt(lambda_f) * base_pred.detach()
            + math.sqrt(1 - lambda_f) * residual_pred
        )
    ...
    def add_noise(
        self, z_base, base_noise, z_residual, composed_noise, noise_scheduler, timesteps
    ):
    ...
    def split_latent_into_base_residual(self, z_0, pl_module, noise_generator):
        if self.random_frame:
            ...
            fr_select = torch.randint(
                0, z_0.shape[1], (bsz,), device=pl_module.device
            ).long()
        ...
        composed_noise = (
            math.sqrt(lambda_f) * base_noise + math.sqrt(1 - lambda_f) * residual_noise
        )  # dimension issue?
...
class NoiseGenerator:
    ...
    def sample_noise(
        ...
        assert (z_0 is not None) != (
            shape is not None
        ), f"either z_0 must be None, or shape must be None. Both provided."
        kwargs = {}
        ...
        if self.mode == "vanilla":
            ...
            shape = list(shape)
            shape[1] = 1
            noise_init = torch.randn(shape, **kwargs)
            shape[1] = F
            noise = torch.zeros(shape, device=noise_init.device, dtype=noise_init.dtype)
            for fr in range(F):
            ...
            for fr in range(F - 1):
                ...
                shifted_noise = torch.cat(
                    [local_copy[:, :, :, shift:, :], local_copy[:, :, :, :shift, :]],
                    dim=3,
                )
                noises.append(
                    math.sqrt(0.2) * shifted_noise
            ...
            init_noise = torch.randn(shape, **kwargs)
            noises = []
