...
class ConcatDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        sample = {ds: self.datasets[ds].__getitem__(idx) for ds in self.datasets}
        return sample
...
class CustomPromptsDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, prompt_cfg: Dict[str, str]):
                ...
                if "videos_root" in prompt_cfg:
                    ...
                    video_path = [
                        str(
                            videos_root
                            / sample["page_dir"]
                            / f"{sample['videoid']}.mp4"
                        )
                        for sample in metadata
                    ]
                else:
                    video_path = [
                        str(sample["page_dir"] / f"{sample['videoid']}.mp4")
                        for sample in metadata
                    ]
        ...
        for prompt in self.prompts:
            transformed_prompts.append(Annotations.clean_prompt(prompt))
