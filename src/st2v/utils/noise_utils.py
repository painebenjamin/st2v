...
def gaussian_smoothing_kernel(shape, kernel_size, sigma, dim=2):
    ...
    pad_length = (
        math.floor((shape[-1] - kernel_size[-1]) / 2),
        math.floor((shape[-1] - kernel_size[-1]) / 2),
        math.floor((shape[-2] - kernel_size[-2]) / 2),
        math.floor((shape[-2] - kernel_size[-2]) / 2),
        math.floor((shape[-3] - kernel_size[-3]) / 2),
        math.floor((shape[-3] - kernel_size[-3]) / 2),
    )
    ...
    def sample_noise(
        ...
        if "mixed_noise" in self.mode:
            ...
            if self.shared_noise_across_chunks and hasattr(self, "e_shared"):
                ...
                e_shared = torch.normal(
                    mean=zero_mean,
                    std=sqrt(std_coeff_shared) * std,
            ...
            for frame in range(shape[frame_idx]):
                ...
                e_ind = torch.normal(
                    mean=zero_mean,
                    std=sqrt(std_coeff_ind) * std,
                    generator=kwargs["generator"] if "generator" in kwargs else None,
                )
                e_inds.append(e_ind)
            noise = torch.cat([e_shared + e_ind for e_ind in e_inds], dim=frame_idx)
        ...
        if "consistI2V" in self.mode and content is not None:
            ...
            if frame_idx == 1:
                assert (
                    content.shape[0] == noise.shape[0]
                    and content.shape[2:] == noise.shape[2:]
                )
                content = torch.concat(
                    [
                        content,
                        content[:, -1:].repeat(
                            1, noise.shape[1] - content.shape[1], 1, 1, 1
                        ),
                    ],
                    dim=1,
                )
                ...
                assert (
                    content.shape[:2] == noise.shape[:2]
                    and content.shape[3:] == noise.shape[3:]
                )
                content = torch.concat(
                    [
                        content,
                        content[:, :, -1:].repeat(
                            1, 1, noise.shape[2] - content.shape[2], 1, 1
                        ),
                    ],
                    dim=2,
                )
            ...
            gaussian_3d = gaussian_smoothing_kernel(
                noise_fft.shape,
                kernel_size=(
                    noise_fft.shape[-3],
                    noise_fft.shape[-2],
            ...
            noise = fft.ifftn(
                fft.ifftshift(noise_fft_shifted_hp + content_noisy_fft_shifted_lp)
            )
            if frame_idx == 1:
                noise = rearrange(noise, "(B C) F W H -> B F C W H", B=B)
            else:
                noise = rearrange(noise, "(B C) F W H -> B C F W H", B=B)
