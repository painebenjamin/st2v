...
def pil_concat(images, labels=None, col=8, fontsize=12):
    ...
    if labels is not None:
        labeled_images = [
            add_label(image, labels[image_idx], fontsize=fontsize)
            for image_idx, image in enumerate(images)
        ]
    else:
    ...
    for row_idx in range(int(np.ceil(len(labeled_images) / col))):
        labeled_images_rows.append(
            pil_concat_h(labeled_images[col * row_idx : col * (row_idx + 1)])
        )
...
def get_first_k_token_head_att_maps(
    atts_normed, k, h, w, output_h=256, output_w=256, labels=None, max_scale=False
):
...
def get_first_k_token_att_maps(
    atts_normed, k, h, w, output_h=256, output_w=256, labels=None, max_scale=False
):
    att_images = []
