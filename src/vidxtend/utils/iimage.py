...
class IImage:
    ...
    def normalized(x, dims=[-1, -2]):
        x = (x - x.amin(dims, True)) / (x.amax(dims, True) - x.amin(dims, True))
        return IImage(x, 0)
    ...
    def torch(self, vmin=-1, vmax=1):
        if self.data.ndim == 3:
            data = self.data.transpose(2, 0, 1) / 255.0
        else:
            data = self.data.transpose(0, 3, 1, 2) / 255.0
    ...
    def __init__(self, x, vmin=-1, vmax=1, fps=None):
        ...
        self.device = "cpu"
        self.fps = fps if fps is not None else (1 if len(self.data) < 10 else 30)
        self.link = None
    ...
    def resize(self, size, *args, **kwargs):
        ...
        return stack(
            [
                IImage(x.pil().resize(size[::-1], *args, resample=resample, **kwargs))
                for x in self
            ]
        )
    ...
    def png(self):
        return IImage(
            np.concatenate([self.data, 255 * np.ones_like(self.data)[..., :1]], -1)
        )
    ...
    def grid(self, nrows=None, ncols=None):
        ...
        rows = [
            np.concatenate(x, 1, dtype=np.uint8) for x in np.array_split(data, nrows)
        ]
    ...
    def heatmap(self, resize=None, cmap=cv2.COLORMAP_JET):
        data = np.stack(
            [
                cv2.cvtColor(cv2.applyColorMap(x, cmap), cv2.COLOR_BGR2RGB)
                for x in self.data
            ]
        )
    ...
    def dilate(self, iterations=1, *args, **kwargs):
        ...
        return IImage(
            (
                binary_dilation(self.data, iterations=iterations, *args, *kwargs)
                * 255.0
            ).astype(np.uint8)
        )
    ...
    def erode(self, iterations=1, *args, **kwargs):
        return IImage(
            (
                binary_erosion(self.data, iterations=iterations, *args, *kwargs) * 255.0
            ).astype(np.uint8)
        )
    ...
    def hull(self):
        ...
        for frame in self.data:
            ...
            convex_hull = cv2.drawContours(canvas, mask_contours, -1, (255, 0, 0), -1)
    ...
    def write(
        self, text, center=(0, 25), font_scale=0.8, color=(255, 255, 255), thickness=2
    ):
    ...
    def append_text(
        ...
        def wrap_text(text, width, _font_scale):
            ...
            if (
                cv2.getTextSize(
                    current_line, cv2.FONT_HERSHEY_COMPLEX, _font_scale, thickness
                )[0][0]
                <= width
            ):
                lines.append(current_line)
            ...
            while height > y_max:
                ...
                if len(text) > 0 and len(text_lines) == 0:
                    ...
                    line_height = cv2.getTextSize(
                        text_lines[0], cv2.FONT_HERSHEY_COMPLEX, _font_scale, thickness
                    )[0][1]
    ...
    def __mul__(self, other):
        if isinstance(other, IImage):
            return IImage(self.data / 255.0 * other.data)
        return IImage(self.data * other / 255.0)

    def __xor__(self, other):
        return IImage(
            0.5 * self.data
            + 0.5 * other.data
            + 0.5 * self.data * (other.data.sum(-1, keepdims=True) == 0)
        )
