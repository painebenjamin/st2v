...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if not final_3d_conv:
            ...
            self.conv_temp = zero_module(
                TemporalConvLayer_Custom(
                    num_frames_conditioning, num_frames, dropout=0.0
                ),
                reset=zero_init,
            )
        ...
        #     num_frames_conditioning, num_frames, kernel_size=3, padding=1)
        # )
    ...
    def forward(self, conditioning, vq_gan=None, controlnet_mask=None):
        ...
        if self.final_3d_conv:
            # embedding = F.silu(embedding)
            embedding = rearrange(
                embedding, "(b f) c h w -> b f c h w", f=self.num_frames_conditioning
            )
...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.use_repeat_context_img = use_repeat_context_img
        ...
        """
        self.conv_in = Conv2DSubChannels(
            in_channels,
        ...
        self.merger = Merger(
            ...
            n_frames_condition=num_frames_conditioning,
    ...
    def forward(
        ...
        if encoder_hidden_states.shape[1] > 77:
            ...
            if self.use_repeat_context_img:
